...
#' filter_regulon - Filter a Regulon Object
#'
#' Subsets a regulon by centroid, by edge likelihood and by absolute
#' correlation. Centroids left without targets are removed.
#'
#' @param regulon A regulon object, as generated by corto()
#' @param tf_list A character vector of centroid names to keep. Default is
#' NULL, i.e. all centroids
#' @param likelihood_threshold Minimum edge likelihood, between 0 and 1.
#' Default is 0
#' @param correlation_threshold Minimum absolute correlation. Default is 0
#' @return A regulon object, or NULL if no edge survives
#' @examples
...
#' filtered<-filter_regulon(regulon,likelihood_threshold=0.5,correlation_threshold=0.9)
#' length(filtered)
#' @export
filter_regulon<-function(regulon,tf_list=NULL,likelihood_threshold=0,correlation_threshold=0){
    if(!is.list(regulon)){
        stop("regulon must be a regulon object, as generated by corto()")
    }
    if(is.null(tf_list)){
        tf_list<-names(regulon)
    } else {
        missing<-setdiff(tf_list,names(regulon))
        if(length(missing)==length(tf_list)){
            stop("None of the requested centroids is present in the regulon")
        }
        tf_list<-intersect(names(regulon),tf_list)
    }
    filtered<-list()
    for(tf in tf_list){
        tfmode<-regulon[[tf]]$tfmode
        likelihood<-regulon[[tf]]$likelihood
        keep<-likelihood>=likelihood_threshold&abs(tfmode)>=correlation_threshold
        if(any(keep)){
            filtered[[tf]]<-list(
                tfmode=tfmode[keep],
    ...
    class(filtered)<-"regulon"
    return(filtered)
}

#' getregulon - Flatten a Regulon Object into a Table
#'
#' Converts a regulon into a data frame of edges, one row per
#' centroid-target pair, optionally written to a tab-separated file.
#'
#' @param regulon A regulon object, as generated by corto()
#' @param tf_list A character vector of centroid names to keep. Default is
#' NULL, i.e. all centroids
#' @param likelihood_threshold Minimum edge likelihood, between 0 and 1.
#' Default is 0
#' @param export A logical value. If TRUE the table is also written to file.
#' Default is FALSE
#' @param filename The file to write when export is TRUE. Default is
#' "regulon.tsv"
#' @return A data frame with columns tf, target, correlation and likelihood
#' @examples
#' load(system.file("extdata","inmat.rda",package="corto",mustWork=TRUE))
#' load(system.file("extdata","centroids.rda",package="corto",mustWork=TRUE))
#' regulon<-corto(inmat,centroids=centroids,nbootstraps=10,p=1e-30,nthreads=2)
...
#' head(edges)
#' @export
getregulon<-function(regulon,tf_list=NULL,likelihood_threshold=0,export=FALSE,
                     filename="regulon.tsv"){
    regulon<-filter_regulon(regulon,tf_list=tf_list,likelihood_threshold=likelihood_threshold)
    if(is.null(regulon)){
        edges<-data.frame(tf=character(0),target=character(0),
                          correlation=numeric(0),likelihood=numeric(0),
                          stringsAsFactors=FALSE)
    } else {
        nedges<-vapply(regulon,function(x){length(x$tfmode)},integer(1))
        edges<-data.frame(
            tf=rep(names(regulon),nedges),
            target=unlist(lapply(regulon,function(x){names(x$tfmode)}),use.names=FALSE),
            correlation=unlist(lapply(regulon,function(x){unname(x$tfmode)}),use.names=FALSE),
            likelihood=unlist(lapply(regulon,function(x){unname(x$likelihood)}),use.names=FALSE),
            stringsAsFactors=FALSE
        )
    }
    if(export){
